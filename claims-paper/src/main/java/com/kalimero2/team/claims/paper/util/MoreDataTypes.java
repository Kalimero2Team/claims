...
public interface MoreDataTypes {
    PersistentDataType<?, UUID[]> UUID_ARRAY = DataType.asArray(new UUID[0], DataType.UUID); // UUID_ARRAY_LIST
    CollectionDataType<List<UUID>, UUID> UUID_LIST = DataType.asList(DataType.UUID);
    CollectionDataType<List<Material>, Material> MATERIAL_LIST = DataType.asList(DataType.asEnum(Material.class));
